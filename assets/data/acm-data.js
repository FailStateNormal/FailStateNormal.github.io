window.ACM_TEMPLATES = [
  ...
  {
    "chapter": "第八章  动态规划",
    "title": "8.3 线性 DP (LIS/LCS/编辑距离)",
    "id": "template-54-8-3-线性DP",
    "code": "// ===== 最长上升子序列 LIS =====\n\n// 二分优化 O(nlogn)：vector + lower_bound\n#include <iostream>\n#include <vector>\n#include <algorithm>\nusing namespace std;\n#define all(x) (x).begin(), (x).end()\nconst int N = 100010;\nint s[N];\nvector<int> q;\n\nint main() {\n    ios::sync_with_stdio(false);\n    cin.tie(nullptr);\n    int n;\n    cin >> n;\n    for (int i = 0; i < n; i++) cin >> s[i];\n    q.push_back(s[0]);\n    for (int i = 1; i < n; i++) {\n        int p = lower_bound(all(q), s[i]) - q.begin();\n        if (p != (int)q.size()) q[p] = s[i];\n        else q.push_back(s[i]);\n    }\n    cout << q.size();\n}\n\n// 二分优化 O(nlogn)：手写二分 + q 数组 (AcWing 标准)\n#include <iostream>\n#include <algorithm>\nusing namespace std;\nconst int N = 100010;\nint n;\nint a[N], q[N];\n\nint main() {\n    scanf(\"%d\", &n);\n    for (int i = 0; i < n; i++) scanf(\"%d\", &a[i]);\n    int len = 0;\n    for (int i = 0; i < n; i++) {\n        int l = 0, r = len;\n        while (l < r) {\n            int mid = l + r + 1 >> 1;\n            if (q[mid] < a[i]) l = mid;\n            else r = mid - 1;\n        }\n        len = max(len, r + 1);\n        q[r + 1] = a[i];\n    }\n    printf(\"%d\\n\", len);\n    return 0;\n}\n\n// 朴素 O(n^2)：f[i] 为以 a[i] 结尾的最长上升子序列长度\n#include <iostream>\n#include <algorithm>\nusing namespace std;\nconst int N = 1010;\nint n;\nint a[N], f[N];\n\nint main() {\n    scanf(\"%d\", &n);\n    for (int i = 1; i <= n; i++) scanf(\"%d\", &a[i]);\n    for (int i = 1; i <= n; i++) {\n        f[i] = 1;\n        for (int j = 1; j < i; j++)\n            if (a[j] < a[i]) f[i] = max(f[i], f[j] + 1);\n    }\n    int res = 0;\n    for (int i = 1; i <= n; i++) res = max(res, f[i]);\n    printf(\"%d\\n\", res);\n    return 0;\n}\n\n// ===== 最长公共子序列 LCS =====\n#include <cstdio>\n#include <algorithm>\nusing namespace std;\nconst int N = 1010;\nchar s1[N], s2[N];\nint f[N][N];\n\nint main() {\n    int n, m;\n    scanf(\"%d%d\", &n, &m);\n    scanf(\"%s%s\", s1 + 1, s2 + 1);\n    for (int i = 1; i <= n; i++)\n        for (int j = 1; j <= m; j++) {\n            f[i][j] = max(f[i - 1][j], f[i][j - 1]);\n            if (s1[i] == s2[j]) f[i][j] = max(f[i - 1][j - 1] + 1, f[i][j]);\n        }\n    printf(\"%d\", f[n][m]);\n    return 0;\n}\n\n// ===== 最短编辑距离 =====\n#include <cstdio>\n#include <algorithm>\nusing namespace std;\nconst int N = 1010;\nint n, m;\nchar s1[N], s2[N];\nint f[N][N];\n\nint main() {\n    scanf(\"%d%s%d%s\", &n, s1 + 1, &m, s2 + 1);\n    for (int i = 1; i <= n; i++) f[i][0] = i;\n    for (int i = 1; i <= m; i++) f[0][i] = i;\n    for (int i = 1; i <= n; i++)\n        for (int j = 1; j <= m; j++) {\n            f[i][j] = min(f[i - 1][j], f[i][j - 1]) + 1;\n            if (s1[i] == s2[j]) f[i][j] = f[i - 1][j - 1];\n            else f[i][j] = min(f[i - 1][j - 1] + 1, f[i][j]);\n        }\n    printf(\"%d\", f[n][m]);\n    return 0;\n}"
  },
  {
    "chapter": "第八章  动态规划",
    "title": "8.4 区间 DP (石子合并)",
    "id": "template-55-8-4-区间DP",
    "code": "// ===== 区间 DP：石子合并 (AcWing 282) =====\n\n// 循环版 (枚举区间长度)\n#include <iostream>\nusing namespace std;\nconst int N = 310;\nint a[N], s[N];\nint f[N][N];\n\nint main() {\n    ios::sync_with_stdio(false);\n    cin.tie(nullptr);\n    int n;\n    cin >> n;\n    for (int i = 1; i <= n; i++) cin >> a[i];\n    for (int i = 1; i <= n; i++) s[i] = s[i - 1] + a[i];\n    for (int len = 2; len <= n; len++)\n        for (int i = 1; i + len - 1 <= n; i++) {\n            int l = i, r = i + len - 1;\n            f[l][r] = 1e9;\n            for (int k = l; k < r; k++)\n                f[l][r] = min(f[l][k] + f[k + 1][r] + s[r] - s[l - 1], f[l][r]);\n        }\n    cout << f[1][n];\n    return 0;\n}\n\n// 递归版 (记忆化)\n#include <iostream>\nusing namespace std;\nconst int N = 310;\nint f[N][N];\nint m[N];\nint s[N];\nint n;\n\nint dp(int l, int r) {\n    if (l >= r) return 0;\n    int& v = f[l][r];\n    if (v) return v;\n    v = 1e9;\n    for (int k = l; k < r; k++)\n        v = min(dp(l, k) + dp(k + 1, r) + s[r] - s[l - 1], v);\n    return v;\n}\n\nint main() {\n    ios::sync_with_stdio(false);\n    cin.tie(nullptr);\n    cin >> n;\n    for (int i = 1; i <= n; i++) {\n        cin >> m[i];\n        s[i] = s[i - 1] + m[i];\n    }\n    cout << dp(1, n);\n}"
  },
  {
    "chapter": "第八章  动态规划",
    "title": "8.5 记忆化搜索 (滑雪)",
    "id": "template-56-8-5-记忆化搜索",
    "code": "// ===== 记忆化搜索：滑雪 (AcWing 901) =====\n// f[x][y] 表示从 (x,y) 出发能滑的最长路径长度\n#include <iostream>\nusing namespace std;\nconst int N = 310;\nint h[N][N];\nint f[N][N];\nint n, m;\nint dx[4] = {-1, 0, 1, 0}, dy[4] = {0, -1, 0, 1};\n\nint dp(int x, int y) {\n    int& v = f[x][y];\n    if (v) return v;\n    v = 1;\n    for (int i = 0; i < 4; i++) {\n        int px = x + dx[i], py = y + dy[i];\n        if (!px || !py || px > n || py > m) continue;\n        if (h[px][py] < h[x][y]) v = max(dp(px, py) + 1, v);\n    }\n    return v;\n}\n\nint main() {\n    ios::sync_with_stdio(false);\n    cin.tie(nullptr);\n    cin >> n >> m;\n    for (int i = 1; i <= n; i++)\n        for (int j = 1; j <= m; j++) cin >> h[i][j];\n    int res = 0;\n    for (int i = 1; i <= n; i++)\n        for (int j = 1; j <= m; j++) res = max(dp(i, j), res);\n    cout << res;\n    return 0;\n}"
  },
  {
    "chapter": "第八章  动态规划",
    "title": "8.6 蒙德里安的梦想 (状压DP)",
    "id": "template-50-8-6-蒙德里安的梦想",
  ...
  {
    "chapter": "第八章  动态规划",
    "title": "8.7 最短哈密顿路径 (状压DP)",
    "id": "template-51-8-7-最短哈密顿路径",
