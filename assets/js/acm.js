...
(function () {
  ...
  window.FSN.initAcmPage = function () {
    ...
    if (!templateList || !catalogList || !searchInput) return;
    if (templateList.dataset.acmReady) return;
    templateList.dataset.acmReady = '1';

    const parsedSections = Array.isArray(window.ACM_TEMPLATES) ? window.ACM_TEMPLATES : [];
